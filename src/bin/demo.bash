...
case "$1" in
  ...
  start)
    ...
    (java -jar *.jar &) > /dev/null ## --spring.profiles.active=prod
  ...
  prod)
    ...
    (java -jar *.jar --spring.profiles.active=prod &) > /dev/null
    echo "Process prod started"
    ;;
  restart)
    $0 stop
    if [ $? = 1 ]
  ...
  debug)
    ...
    (java -server -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n -jar *.jar &)  > /dev/null
