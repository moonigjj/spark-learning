package com.example.spark.ch04
...
class RunRDF(private val spark: SparkSession) {
  ...
  def simpleDecisionTree(trainData: DataFrame, testData: DataFrame): Unit = {
    ...
    val fl = evaluator.setMetricName("fl").evaluate(predictions)
    println(accuracy)
    println(fl)

    val predictionRDD = predictions
      .select("predication", "Cover_Type")
      .as[(Double, Double)].rdd
    val multiclassMetrics = new MulticlassMetrics(predictionRDD)
    println(multiclassMetrics)

    val confusionMatrix = predictions
      .groupBy("Cover_Type")
      .pivot("predication", (1 to 7))
      .count()
      .na.fill(0.0)
      .orderBy("Cover_Type")
